<?php
/*
Plugin Name: JK Event Calendar
Version: 1.5.55
Author: Jalal Khan
Author URI: https://example.com
*/

$wd_spider_calendar_version="1.9.99";
...
function featured_event_shortcode() {
 global $wpdb;
    $rowpic = $wpdb->get_row("SELECT pic FROM " . $wpdb->prefix . "spidercalendar_event WHERE featured = '1' ORDER BY rand() LIMIT 1");
	//var_dump($rowpic);
	return '<div class="col-12 featured_pic"><img src="'.$rowpic->pic.'" class="imgf" /><div class="col-12 featured_txt"><h2>Featured Events</h2></div></div>';
	
}
add_shortcode('featured_event', 'featured_event_shortcode');

function intrested_event_shortcode() {
 global $wpdb;
    $rowCat = $wpdb->get_results("SELECT * FROM " . $wpdb->prefix . "spidercalendar_event_category");
	//var_dump($rowpic);
	 ?>
     <select name="intrested" class="intrevent" onchange="showbigcalendar('bigcalendar1', 'http://ede.a81.myftpupload.com/wp-admin/admin-ajax.php?action=spiderbigcalendar_month&amp;theme_id=13&amp;calendar=1&amp;select=month,&amp;date=2017-11&amp;many_sp_calendar=1&amp;cat_id='+this.value+'&amp;cat_ids&amp;widget=0','1','0')">
...
add_shortcode('intrested_event', 'intrested_event_shortcode');
